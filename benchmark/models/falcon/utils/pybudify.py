                ...
                for layer_num in range(len(self.bound_module.layers)):
                    pybuda.config.insert_fracture_group(
                        [
                            ...
                            (f"matmul_{23+layer_num*offset}", pybuda.k_dim, factor),
                        ]
                    )
                    ...
                    if layer_num < len(self.bound_module.layers) // 2:
                        pybuda.config.insert_fracture_group(
                            [
                                ...
                                (f"matmul_{23+layer_num*offset}", pybuda.k_dim, 2),
                            ]
                        )
                ...
                if fracture_mlp > 0:
                    ...
                    for layer_num in range(len(self.bound_module.layers)):
                        ...
                        pybuda.config.override_op_placement(
                            f"concatenate_{48+mlp_offset}.dc.concatenate.2", chip_id=1, temporal_epoch_break=True
                        )
                        ...
                        ops = [[f"matmul_{18 + mlp_offset}", f"matmul_{23 + mlp_offset}"]]
