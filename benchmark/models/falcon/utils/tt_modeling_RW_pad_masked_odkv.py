...
class PaddedAttentionTT(nn.Module):
    ...
    def make_pad_weights(self, make_kvs=False):
        ...
        self.wq.weight.data = self.mask_weight(
            self.wq.weight.data, self.original_hidden_size, self.original_hidden_size
        )
        self.dense.weight.data = self.mask_weight(
            self.dense.weight.data, self.original_hidden_size, self.original_hidden_size
        )
        if make_kvs:
...
class PaddedMLP(nn.Module):
    ...
    def make_pad_weights(self):
        ...
        self.dense_4h_to_h.weight.data = self.mask_weight(
            self.dense_4h_to_h.weight.data, self.original_hidden_size, self.original_hidden_size * 4
        )
        self.dense_h_to_4h.weight.data = self.mask_weight(
            self.dense_h_to_4h.weight.data, self.original_hidden_size * 4, self.original_hidden_size
        )
...
class SequentialCaller(nn.Module):
    ...
    def forward(
        self, hidden_states, cos, sin, attention_mask=None, kv_read_mask=None, kv_write_mask=None, *past_key_values
    ):
        result = []
...
class PaddedLayerNorm(nn.Module):
    ...
    def __init__(self, normalized_shape, pad_amt, eps=1e-05, elementwise_affine: bool = True, device=None, dtype=None):
        super().__init__()
    ...
    def main_forward_part(
        ...
        **deprecated_arguments,
        ...
        if attention_mask is not None:
            ...
            if self.user_rows >= 1:
                ...
                att_mask_stacked = torch.cat(
                    [attention_mask[:, [n], :, :] for n in range(attention_mask.shape[1])], dim=3
                )
        ...
        if past_key_values is not None and attention_mask is not None:
            ...
            # import pdb; pdb.set_trace()
            outputs = self.blocks(hidden_states, cos, sin, att_mask_stacked, kv_read_mask, kv_write_mask, *flattened_kv)
