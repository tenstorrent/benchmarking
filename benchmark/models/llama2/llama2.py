...
@benchmark_model(configs=["7b", "7b-chat", "70b-chat"])
def llama2(
    ...
    if task == "na":
        ...
        def eval_fn(outputs, labels):
            ...
            for pred, input in zip(pred_labels, eval_set):
                ...
                test_case = (
                    f"llama2-{config}-tt-{num_tokens}tk" if device == "tt" else f"llama2-{config}-gpu-{num_tokens}tk"
                )
            ...
            with open(f"benchmark/models/llama2/results/output_{test_case}.json", "w") as outfile:
