...
@benchmark_model(configs=["7b", "7b-instruct", "8x7b-instruct"])
def mistral(
    ...
    if task == "na":
        ...
        top_p_enable = 0
        max_new_tokens = min_new_tokens = 1
    elif task == "hellaswag":
        max_new_tokens = 1
        min_new_tokens = 1
        ...
        if task == "hellaswag":
            ...
            def model_wrapper(batch):
                ...
                for prompt in batch:
                    ...
                    outputs.append(llm_model(input_ids, labels=input_ids).logits.squeeze(0))
                return outputs
    ...
    if task == "na":
        ...
        def eval_fn(outputs, labels):
            ...
            for b_out, b_lbl in zip(outputs, labels):
                for out, lbl in zip(b_out, b_lbl):
                    ...
                    eval_out = out.squeeze(0)[start_idx:end_idx, :].cpu()
        ...
        # AlpacaEval dataset
        eval_set = load_dataset("tatsu-lab/alpaca_eval", "alpaca_eval", split="eval[:256]")
        ...
        def eval_fn(outputs, labels):
            ...
            for pred, input in zip(pred_labels, eval_set):
                ...
                test_case = (
                    f"mistral-{config}-tt-{num_tokens}tk" if device == "tt" else f"mistral-{config}-gpu-{num_tokens}tk"
                )
            ...
            with open(f"benchmark/models/mistral/results/output_{test_case}.json", "w") as outfile:
