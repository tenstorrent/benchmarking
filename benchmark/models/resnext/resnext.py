...
@benchmark_model(configs=["14", "26", "50", "101"])
def resnext(training: bool, task: str, config: str, microbatch: int, device: str, data_type: str):
    ...
    if config == "14":
        ...
        pybuda.config.override_dram_parameters("conv2d_1372.dc.matmul.8", True)
        pybuda.config.override_dram_parameters("conv2d_1428.dc.matmul.8", True)
        pybuda.config.override_dram_parameters("conv2d_1471.dc.matmul.8", True)
    else:
    ...
    if task == "na":
        ...
        if device == "tt":
            model = {"device": pybuda.PyTorchModule(f"pt_resnext_{config}", model)}
        else:
