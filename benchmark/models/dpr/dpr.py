...
@benchmark_model(configs=["context", "question"])
def dpr(training: bool, task: str, config: str, microbatch: int, device: str, data_type: str):
    ...
    if task == "na":
        ...
        # Load model
        cfg = DPRConfig.from_pretrained(model_name)
        if config == "context":
            model = DPRContextEncoder(config=cfg)
        elif config == "question":
            model = DPRQuestionEncoder(config=cfg)
        ...
        if device == "tt":
            model = {"device": pybuda.PyTorchModule("pt_dpr_na", model)}
        else:
